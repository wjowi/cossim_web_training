...
import { Maximize, Package, CheckCircle, XCircle, Clock, User, MapPin, RotateCcw, AlertCircle, ArrowRight, Trash2 } from "feather-icons-react";
...
export default function DCSingleScan() {
  ...
  const { fetchShipmentOrder, handleUpdateShipmentStatus, fetchShipmentOrderStatus, orderStatuses, dcCode } = useShipment();
  ...
  useEffect(() => { fetchShipmentOrderStatus(); }, [fetchShipmentOrderStatus]);

  // Options from order-specific transitions
  const transitionOptions = resolvedOrder?.StatusTransitionArray?.map((t) => ({
    value: t.toOrderStatusID,
    label: t.actionName,
    transition: t,
  })) || [];

  // Options from global orders status endpoint
  const globalStatusOptions = orderStatuses?.map((s) => ({
    value: s.orderStatusID,
    label: s.statusName,
    isGlobal: true,
  })) || [];

  // Combine options (prioritize transitions if available)
  const statusOptions = transitionOptions.length > 0 ? transitionOptions : globalStatusOptions;
  ...
  const handleScan = useCallback(async () => {
    const orderNO = scanInput.trim();

    // Synchronous lock — guards against a scanner sending Enter more than
    // once (or a stray repeat keydown) triggering a duplicate resolve call.
    ...
    try {
      ...
      if (!order || response?.Error) {
        setResolveError(response?.Message || "Package not found. Check the barcode and try again.");
        return;
      }
  ...
  const handleUpdateStatus = useCallback(async (e) => {
    ...
    try {
      const payload = {
        ...
        notes,
        dcCode: dcCode || resolvedOrder.OriginDCCode || resolvedOrder.DestinationDCCode,
      };
  ...
  }, [resolvedOrder, selectedStatus, notes, dcCode, handleUpdateShipmentStatus, resetForm]);
      ...
      <div className="page-header mb-4">
        <div className="add-item d-flex">
          <div className="page-title">
            ...
            <h6 className="text-muted">Scan a package and update its status</h6>
          </div>
        </div>
          ...
          <Card className="border-0 shadow-sm rounded-3 mb-4">
            <Card.Header className="bg-transparent border-bottom px-4 pt-4 pb-3">
              <div className="d-flex align-items-center gap-2">
                <span className="badge bg-primary rounded-circle" style={{ width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>1</span>
            ...
            <Card.Body className="p-4">
              ...
              <small className="text-muted mt-2 d-block">
                Physical scanners send Enter after reading — just aim and scan.
              </small>
              {resolveError && (
            ...
            <Card className="border-0 shadow-sm rounded-3 mb-4" style={{ borderLeft: "4px solid #198754" }}>
              <Card.Header className="bg-transparent border-bottom px-4 pt-4 pb-3">
                <div className="d-flex align-items-center justify-content-between">
                  <div className="d-flex align-items-center gap-2">
                    <span className="badge bg-success rounded-circle" style={{ width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>2</span>
              ...
              <Card.Body className="p-4">
                ...
                <div className="rounded-3 p-3 mb-4 bg-light">
                  <Row className="g-2 align-items-center">
                    ...
                    <Col>
                      ...
                      {resolvedOrder.OriginDCCode && (
                        <div className="small text-muted">
                          <strong>DC:</strong> {resolvedOrder.OriginDCCode}
                          {resolvedOrder.DestinationDCCode && <> → {resolvedOrder.DestinationDCCode}</>}
                        </div>
                      )}
                    </Col>
                    <Col xs="auto">
                      <Badge bg="primary" className="px-3 py-2 rounded-pill">{resolvedOrder.StatusName || "Unknown"}</Badge>
                    </Col>
                  </Row>
                </div>
                ...
                <Form onSubmit={handleUpdateStatus}>
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-semibold">Select New Status <span className="text-danger">*</span></Form.Label>
                    {statusOptions.length > 0 ? (
                      <SSRSelect
                        ...
                        instanceId="dc-single-scan-status"
                      />
                    ) : (
                  ...
                  {selectedStatus?.transition && (
                    <Alert variant="info" className="py-2 mb-3 small">
                      ...
                      {selectedStatus.transition.requiresPickupFlow && <div>• Requires pickup flow</div>}
                      {selectedStatus.transition.requiresCOD && <div>• Requires COD</div>}
                      {selectedStatus.transition.requiresPOD && <div>• Requires POD</div>}
                    </Alert>
                  )}

                  <Form.Group className="mb-4">
                    <Form.Label className="fw-semibold">Notes <span className="text-muted fw-normal">(optional)</span></Form.Label>
                    <Form.Control
                      as="textarea"
                      rows={2}
                      placeholder="e.g. Received at DC, forwarded to next hub…"
                      value={notes}
                  ...
                  <div className="d-flex gap-2 justify-content-end">
                    <Button variant="light" onClick={resetForm} disabled={submitting}>Cancel</Button>
                    <Button type="submit" variant="primary" disabled={submitting || !selectedStatus} style={{ minWidth: 140 }}>
                      {submitting
                        ...
                        : <><CheckCircle size={14} className="me-2" />Update Status</>}
                    </Button>
                  </div>
                </Form>
              </Card.Body>
                ...
                <div className="d-flex flex-column gap-2">
                  {sessionLog.map((entry) => (
                    ...
                    >
                      ...
                      <div className="small text-muted"><User size={11} className="me-1" />{entry.recipient}</div>
                      <div className="small text-muted">→ {entry.statusLabel}</div>
                      {entry.result === "error" && <div className="small text-danger mt-1">{entry.message}</div>}
                      <div className="small text-muted mt-1"><Clock size={11} className="me-1" />{entry.time}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            {sessionLog.length > 0 && (
              <Card.Footer className="bg-transparent border-top px-4 py-3">
                <Row className="text-center g-0">
                  <Col><div className="fw-bold">{sessionLog.length}</div><div className="small text-muted">Total</div></Col>
                  <Col><div className="fw-bold text-success">{successCount}</div><div className="small text-muted">Success</div></Col>
                  <Col><div className="fw-bold text-danger">{failCount}</div><div className="small text-muted">Failed</div></Col>
                </Row>
              </Card.Footer>
            )}
          </Card>
