...
export default function DashboardPage(){
  ...
  return <main className={styles.page}><div className={styles.dashboard}>
    ...
    <div className={`${styles.modalBackdrop} ${!selectedTask?styles.hidden:""}`} onMouseDown={event=>{if(event.target===event.currentTarget)setSelectedTask(null)}} aria-hidden={!selectedTask}><section className={`${styles.modal} ${styles.packagesModal}`} role="dialog" aria-modal={Boolean(selectedTask)}><header><div><small>CURRENT TASKS</small><h2>{selectedTask?.label||"TASK MANAGEMENT"}</h2></div><button onClick={()=>setSelectedTask(null)} aria-label="Close task dialog"><X/></button></header><div className={styles.packagesModalBody}>{selectedTask&&<PackagesList key={selectedTask.key} initialTask={selectedTask.key}/>}</div></section></div>
